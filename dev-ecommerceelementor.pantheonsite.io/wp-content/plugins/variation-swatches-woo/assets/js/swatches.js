(function ($) {
  ...
  const SW = {
    ...
    chooseOption() {
      ...
      getForm.find(".disable-to-select").removeClass("disable-to-select");
      const td = option.closest("td");
      const checkTdSelected = td.find(".cfvsw-hidden-select select").val();
      if ("" !== checkTdSelected) {
    ...
    swatchesOptions: (getForm, getFormData) => {
      const getTdAvoidCurrent = getForm.find("td").not(".disable-to-select");
  ...
  function updateThumbnail(swatch, imageData) {
    ...
    if (0 === listItem.find(".cfvsw-original-thumbnail").length) {
      ...
      listItem.find(".cfvsw-original-thumbnail").html(originalThumbnail);
    }
  ...
  function updatebuttonData(variant, variation) {
    ...
    const data = {};
    const button = variant.parents("li").find(".cfvsw_ajax_add_to_cart");
